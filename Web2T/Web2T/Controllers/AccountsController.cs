...
namespace Web2T.Controllers
{
    ...
    public class AccountsController : Controller
    {
        ...
        [HttpPost]
        public IActionResult ChangeAddress(Customer model)
        {
            try
            {
                var taikhoanID = HttpContext.Session.GetString("CustomerId");
                ...
                int customerID = Convert.ToInt32(taikhoanID);
                var found = _context.Customers.AsNoTracking().SingleOrDefault(x => x.CustomerId == customerID);
                if (found == null) return RedirectToAction("Login", "Accounts");

                found.Address = model.Address;
                //_context.Update(found);
                 _context.Entry(found).State = EntityState.Modified;
                _context.SaveChanges();
                _notyfService.Success("Updated Address Success !");
                return RedirectToAction("Dashboard", "Accounts");
            }
            catch (Exception ex)
            {
            ...
            _notyfService.Success("Cập nhật địa chỉ không thành công");
            return View(model);
        }
    }
}
