...
namespace Web2T.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        public IActionResult Index(MuaHangVM muaHang)
        {
            ...
            try
            {
                ...
                foreach (var item in cart)
                {
                    ...
                    {
                        // var discount = (Model.Discount / 100) * Model.Price;
                        var giamgia = (double)item.product.Discount * 0.01;
                        var sotiengiam = item.product.Price * giamgia;
                        orderDetail.Price = (int?)(decimal)(item.product.Price - sotiengiam);
                    }
                    else
                    {
        ...
        public string GetNameLocation(int locationid)
        {
            try
            {
                var location = _context.Locations.AsNoTracking().SingleOrDefault(x => x.LocationId == locationid);
                if (location != null)
                {
